...
def model(X, Y):
	...
	with tf.variable_scope('prestack'):
		...
		x_fcs_tied_proper = tf.reshape(tf.transpose(x_fcs_tied, perm=[1,0,2]),[-1,5*484])


	z1 = fc_layers(x_fcs_tied_proper)
	...
	##Restore cldnn
	saver.restore(sess,'D:/fyndML/Models/cnn/cnn.ckpt')
...
if __name__ == '__main__':
	...
	with open('../model_def/cnn.json','r') as f:
		defs = json.load(f)
